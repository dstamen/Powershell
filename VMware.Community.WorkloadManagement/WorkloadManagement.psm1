...
Function Get-WorkloadManagementClusterCompatibility {
	<#
		...
		===========================================================================
		.SYNOPSIS
			This function  provides operations to get Namespace-related compatibility information for clusters in this vCenter.
		.DESCRIPTION
			Function to return details about the Supervisor Clusters
		.EXAMPLE
			Connect-CisServer -Server 192.168.1.51 -User administrator@vsphere.local -Password VMware1!
...
Function Get-WorkloadManagementHostsConfig {
	<#
		.NOTES
		===========================================================================
        Created by:    David Stamen
		...
		===========================================================================
		.SYNOPSIS
			The Function provides operations to retrieve information about vSphere Namespaces support and licensing.
		.DESCRIPTION
			Function to return details about the Namespaces
		.EXAMPLE
			Connect-CisServer -Server 192.168.1.51 -User administrator@vsphere.local -Password VMware1!
	...
	$servers = $Global:DefaultCisServers
	foreach ($server in $servers) {
		$systemUpdateAPI = Get-CisService 'com.vmware.vcenter.namespace_management.hosts_config' -Server $server.Name
		for ($i=0; $i -lt $systemUpdateAPI.count; $i++) {
			$results = $systemUpdateAPI.Get($i);
			$summaryResult = [pscustomobject] @{
				"Server" = $server.Name;
				"Namespaces Licensed" = $results.namespaces_licensed;
				"Namespaces Supported" = $results.namespaces_supported
			}
		}
		$summaryResult
    }
